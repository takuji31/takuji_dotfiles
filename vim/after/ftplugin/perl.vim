...
iabbrev vc, state $validator = Data::Validator->new(<CR>)->with(qw(Method Croak));<CR>my ($class, $args) = $validator->validate(@_);<Up><Up><End><CR><C-R>=Eatchar('\s')<CR>
iabbrev vs, state $validator = Data::Validator->new(<CR>)->with(qw(Method Croak));<CR>my ($self, $args) = $validator->validate(@_);<Up><Up><End><CR><C-R>=Eatchar('\s')<CR>
iabbrev v, state $validator = Data::Validator->new(<CR>)->with(qw(Croak));<CR>my $args = $validator->validate(@_);<Up><Up><End><CR><C-R>=Eatchar('\s')<CR>
